...
function genCode(handler) {
    ...
    for (let i = 0; i < classCnt; i++) {
        ...
        for (let j = 0; j < memberCnt; j++) {
            ...
            writer.writeln('public %s %s;', memberInfo.res == null ? memberInfo.type : handler.ToFilename(memberInfo.res.owner.name) + "." + memberInfo.res.name, memberInfo.varName);
        }
        ...
        writer.writeln('UIPackage.CreateObjectAsync("%s", "%s", (go) => tcs.SetResult(go as %s));', handler.pkg.name, classInfo.className, classInfo.resName);
