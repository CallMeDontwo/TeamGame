...
namespace FairyGUI
{
    ...
    public class StageCamera : MonoBehaviour
    {
        ...
        /// 
        /// </summary>
        private bool _constantSize = DefaultConstantSize;
        public bool constantSize
        {
            get { return this._constantSize; }
            set { this._constantSize = value; this.ApplyModifiedProperties(); }
        }

        private float _cameraSize = DefaultCameraSize;
        public float cameraSize
        {
            get { return this._cameraSize; }
            set { this._constantSize = true; this._cameraSize = value; this.ApplyModifiedProperties(); }
        }
        ...
        /// </summary>
        [NonSerialized]
        private float _unitsPerPixel = DefaultUnitsPerPixel;
        public float unitsPerPixel
        {
            get { return this._unitsPerPixel; }
            set { this._constantSize = false; this._unitsPerPixel = value; this.ApplyModifiedProperties(); }
        }
        ...
        public static bool DefaultConstantSize = true;
        ...
        void OnScreenSizeChanged(int newWidth, int newHeight)
        {
            ...
            if (constantSize)
            {
                _unitsPerPixel = _cameraSize * 2 / screenHeight;
                cachedCamera.orthographicSize = _cameraSize;
            }
            else
            {
                _cameraSize = screenHeight / 2 * unitsPerPixel;
                cachedCamera.orthographicSize = _cameraSize;
            }
        ...
        void OnDestroy()
        {
            if (Camera.main)
            {
                ...
                if (mainCameraData)
                {
                    mainCameraData.cameraStack.Remove(cachedCamera);
                }
            }
        }
        ...
        public static Camera CreateCamera(string name, int cullingMask)
        {
            ...
            if (Camera.main)
            {
                ...
                if (cameraData && mainCameraData)
                {
                    ...
                    camera.allowHDR = Camera.main.allowHDR;
                    camera.allowMSAA = Camera.main.allowMSAA;
