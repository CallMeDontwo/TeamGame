...
namespace ET
{
    ...
    {
        [Invoke(InvokeType_Timer.SessionAcceptTimeout)]
        ...
        {
            self.Timer = self.Root().GetComponent<TimerComponent>().NewOnceTimer(TimeInfo.Instance.ServerNow() + 5000, InvokeType_Timer.SessionAcceptTimeout, self);
        }
