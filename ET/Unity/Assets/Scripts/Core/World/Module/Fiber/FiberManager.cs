...
namespace ET
{
    ...
    public class FiberManager : Singleton<FiberManager>, ISingletonAwake, ISingletonReverseDispose
    {
        ...
        public int Count => this.fibers.Count;
            ...
            try
            {
                ...
                fiber.ThreadSynchronizationContext.Post(async () =>
                {
                    try
                    {
                        ...
                        await EventSystem.Instance.Invoke<FiberCreate, ETTask>((long)sceneType, new FiberCreate() { Fiber = fiber });
                        tcs.SetResult(true);
                    }
                    catch (Exception e)
                    {
        ...
        {
            return await this.Create(schedulerType, Interlocked.Increment(ref this.idGenerator), zone, sceneType, name);
        }
