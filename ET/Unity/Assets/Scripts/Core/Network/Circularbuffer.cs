...
namespace ET
{
    public class CircularBuffer : Stream
    {
        ...
        public void Peek(Stream stream, int count)
        {
            if (count > this.Length)
            {
                throw new Exception($"bufferList length < count, {this.Length} {count}");
            }

            int alreadyCopyCount = 0;
            int firstIndex = this.FirstIndex;
            Queue<byte[]>.Enumerator first = this.bufferQueue.GetEnumerator();
            first.MoveNext();
            while (alreadyCopyCount < count)
            {
                int n = count - alreadyCopyCount;
                if (this.ChunkSize - firstIndex > n)
                {
                    stream.Write(first.Current, firstIndex, n);
                    firstIndex += n;
                    alreadyCopyCount += n;
                }
                else
                {
                    stream.Write(first.Current, firstIndex, this.ChunkSize - firstIndex);
                    alreadyCopyCount += this.ChunkSize - firstIndex;
                    firstIndex = 0;
                    first.MoveNext();
                }
            }
        ...
        public void Read(Stream stream, int count)
        {
            ...
            while (alreadyCopyCount < count)
            {
                ...
                {
                    stream.Write(this.First, this.FirstIndex, this.ChunkSize - this.FirstIndex);
        ...
        public int Peek(byte[] buffer, int offset, int count)
        {
            ...
            while (alreadyCopyCount < count)
            {
                ...
                if (this.ChunkSize - firstIndex > n)
                {
                    Array.Copy(first.Current, firstIndex, buffer, alreadyCopyCount + offset, n);
                    firstIndex += n;
                    alreadyCopyCount += n;
                }
                else
                {
                    Array.Copy(first.Current, firstIndex, buffer, alreadyCopyCount + offset, this.ChunkSize - firstIndex);
                    alreadyCopyCount += this.ChunkSize - firstIndex;
                    firstIndex = 0;
                    first.MoveNext();
                }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            while (alreadyCopyCount < count)
            {
                ...
                {
                    Array.Copy(this.First, this.FirstIndex, buffer, alreadyCopyCount + offset, this.ChunkSize - this.FirstIndex);
                    alreadyCopyCount += this.ChunkSize - this.FirstIndex;
                    this.FirstIndex = 0;
                    this.RemoveFirst();
                }
