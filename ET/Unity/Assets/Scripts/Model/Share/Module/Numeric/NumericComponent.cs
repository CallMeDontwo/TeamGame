...
namespace ET
{
    ...
    public static class NumericComponentSystem
    {
        ...
        /// <summary>从数值类型数组和数值数组批量初始化（按索引一一对应）</summary>
        public static void InitFromArrays(this NumericComponent self, int[] numericTypes, int[] numericValues)
        {
            if (numericTypes == null || numericValues == null) return;
            int len = System.Math.Min(numericTypes.Length, numericValues.Length);
            for (int i = 0; i < len; i++)
            {
                self.Set(numericTypes[i], numericValues[i]);
            }
        }
